...
public class SignedClient {
	...
	public void error_report(final ActivityBase activity,JSONObject result){
		if(result == null){
	...
	private byte[] send_request(HttpRequestBase request){
		...
		for( Header header : request.getAllHeaders() ){
			Log.d(TAG,String.format("%s %s",header.getName(), header.getValue() ));
		}
		...
		for(int nTry=0;nTry<10;++nTry){
    		...
    		try{
	        	...
	        	try{
	        		...
	        		Log.d(TAG,"read failed. status="+response.getStatusLine()+", url="+request.getURI());
	        	}finally{
	        		in.close();
	        	}
